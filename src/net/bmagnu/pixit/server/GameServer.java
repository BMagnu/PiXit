...
public class GameServer {
	...
	public synchronized int reconnectPlayer(Player player) {
		...
		// Delay renotification
		Server.addToQueue(() -> {
			player.proxy.notifyNewGamestate(GameState.STATE_WAITING_FOR_CZAR);
			...
			Server.addToQueue(() -> {
				// TODO May fail at high network delay if colliding with above image crawling
				// The first notifyNewGamestate to STATE_WAITING_FOR_CZAR initializes the crawling of a players image
				// This needs to complete before we continue, so 1. we delay sending the new gamestate, and 2. we add it to the queue as to process other requests first
				try {
					Thread.sleep(500);
				} catch (InterruptedException e) {
				}
				
				player.proxy.notifyNewGamestate(player.state);
	...
	protected void processInitialization() {
		...
		for (int i = 0; i < players.size(); i++) {
			...
			p.proxy.notifyMiscInfo(findPlayerById(currentPlayer).name, images.size(), images.size());
		}

		// Send each player all player info
		for (int i = 0; i < players.size(); i++) {
			if (i == currentPlayer)
				findPlayerById(i).notifyNewGamestate(GameState.STATE_WAITING_FOR_CZAR_YOU);
			else
				findPlayerById(i).notifyNewGamestate(GameState.STATE_WAITING_FOR_CZAR);
		}
	}
	...
	protected void processAllImagesPlayed() {
		...
		for (int i = 0; i < players.size(); i++) {
			...
			if (i != currentPlayer)
				p.notifyNewGamestate(GameState.STATE_WAITING_FOR_GUESS);
			else
				p.notifyNewGamestate(GameState.STATE_WAITING_FOR_GUESS_CZAR);
	...
	protected void processAllImagesGuessed() {
		...
		int correctImage = currentImages.entrySet().stream().filter((entry) -> entry.getValue() == currentPlayer)
				.findFirst().get().getKey().imageId;
		...
		if (!Settings.PLAY_FULL_ROUNDS || currentPlayer == 0) {
			...
			if (freeImages.size() < neededImages || rounds >= Settings.MAX_ROUNDS) {
				for (int i = 0; i < players.size(); i++) {
					findPlayerById(i).notifyNewGamestate(GameState.STATE_GAME_OVER);
				}
		...
		for (int i = 0; i < players.size(); i++) {
			...
			if (i == currentPlayer)
				p.notifyNewGamestate(GameState.STATE_WAITING_FOR_CZAR_YOU);
			else
				p.notifyNewGamestate(GameState.STATE_WAITING_FOR_CZAR);
		}
	}
	...
	protected void processCzarTheme(String theme) {
		for (int i = 0; i < players.size(); i++) {
			Player p = findPlayerById(i);
			p.notifyNewGamestate(GameState.STATE_WAITING_FOR_CARDS);
